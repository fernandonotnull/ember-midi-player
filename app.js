if (window.FileReader) {
	...
	App.PlayerControlsComponent = Ember.Component.extend({
		...
		isPaused: false,
		playerVolume: 100,
		didInsertElement: ( function (_this) {
			...
			$( "#slider" ).slider({
			      ...
			      max: 100,
			      value: 100,
				  step: this.playerVolumeStep,
			      slide: (function (that){

			      	return function( event, ui ) {
			      	that.set('playerVolume',ui.value);
			        that.sendAction('changeVolume',ui.value);
			      }
			   })(this)
			});  
			}
	...
	App.MidiPlayerComponent = Ember.Component.extend({
		...
		loadMidiFile: function () {
			...
			if ( !this.get('checkType') ||  !this.get('checkType').match(/^audio\/mid/)) {

				this.set('errorMidiType',true);	
				...
				try {
					...
					this.set('songDuration',MIDI.Player.endTime/1000);
					this.set('errorMidiType',false);
		...
		*/
		...
		actions: {
			...
			cambioVolumen: function (volumen) {
				switch (MIDI.api) {
					case 'webmidi':
					case 'audiotag':
						for (var i=0;i<16;i++) {
							MIDI.setVolume(i,volumen*127/100);
						}			
					break;
					default:
						MIDI.setVolume(0,volumen*127/100);
				}
			},
	...
	document.write('<div class="bg-danger"> Oh, oh!. No es posible usar este navegador para nuestra aplicación. Use otro navegador o actualice a la última versión. Gracias.</div>');
}
